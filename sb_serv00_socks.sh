...
cd ${WORKDIR}
TMPDIR="${WORKDIR}" exec ${WORKDIR}/npm -s ${NEZHA_SERVER}:${NEZHA_PORT} -p ${NEZHA_KEY} ${NEZHA_TLS} --report-delay 4 --disable-auto-update --disable-force-update >/dev/null 2>&1
EOF
...
run_sb() {
  if [ -e npm ]; then
    nohup ${WORKDIR}/nezha.sh >/dev/null 2>&1 &
	  sleep 2
    pgrep -x "npm" > /dev/null && green "npm is running" || { red "npm is not running, restarting..."; pkill -x "npm" && nohup ${WORKDIR}/nezha.sh >/dev/null 2>&1 & sleep 2; purple "npm restarted"; }
    else
